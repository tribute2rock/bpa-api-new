SELECT id, requestKey, request, category, name, email, requestSenderType, requestedDate, statusId
FROM (
         select r.id,
                r.requestKey,
                f.name           request,
...
WHERE statusId != 5 and status = ':status' and requestSenderType = 'user'
order by id DESC
