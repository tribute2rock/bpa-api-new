SELECT id, requestKey, request, category, customer, account, mobile, requestSenderType, requestedDate, statusId
FROM (
         select r.id,
                r.requestKey,
                f.name           request,
...
WHERE statusId != 5 and status = ':status' and requestSenderType = 'customer' and (requestKey like ':value%' or request like ':value%' or customer like ':value%' or mobile like ':value%' or account like ':value%')
order by id DESC
